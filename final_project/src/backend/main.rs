...
async fn form_handler() -> impl IntoResponse {
    let html_contents = tokio::fs::read_to_string("src/backend/post.html").await.unwrap();
    Html(html_contents)
}
...
async fn answer_form_handler() -> impl IntoResponse {
    let html_contents = tokio::fs::read_to_string("src/backend/answers_form.html").await.unwrap();
    Html(html_contents)
}
